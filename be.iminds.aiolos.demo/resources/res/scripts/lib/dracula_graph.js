...
Graph.Renderer.Raphael = function(element, graph, width, height) {
    ...
    this.dragger = function (e) {
    	selfRef.isSelected && selfRef.isSelected.set.attr("stroke-width", "1");
    	selfRef.isSelected && selfRef.isSelected.set.attr("font-weight", "normal");
    	selfRef.isSelected = this;
    	this.set && this.set.attr("stroke-width", "3");
    	this.set && this.set.attr("font-weight", "bold");
    	if(!this.set[0].fixed){ // allow fixed nodes
	        this.dx = e.clientX;
	        this.dy = e.clientY;
	        selfRef.isDrag = this;
	   
	        this.set && this.set.animate({"fill-opacity": .1}, 200); // dont bring to front for aiolos && this.set.toFront();
...
Graph.Renderer.Raphael.prototype = {
    ...
    drawEdge: function(edge) {
        ...
        if(!edge.connection) {
            ...
            edge.connection = this.r.connection(edge.source.ellipse, edge.target.ellipse, edge.style); // use ellipse instead of shape for better edge render
            return;
        }
